...
import time
from ddqn import plot_smooth_reward
...
class DRQN:
    ...
    def update(self,
        ...
        q_next = self.Q.forward(next_states, max_seq,
                                batch_size).max(1)[0].view(-1, 1)
...
class Recurrent_Memory_ReplayBuffer:
    ...
    def sample(self, batch_size, max_seq):
        # sample episodic memory
        states, actions, rewards, next_states, dones = [], [], [], [], []
        for _ in range(batch_size):
            finish = random.randint(max_seq, self.size() - 1)
            begin = finish - max_seq
            data = []
...
env.seed(0)
torch.manual_seed(0)
replay_buffer = Recurrent_Memory_ReplayBuffer(buffersize)

state_dim = env.observation_space.shape[0]
hidden_dim = 128
action_dim = env.action_space.n

agent = DRQN(state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon,
             device)
        ...
        for i_episodes in range(int(num_episodes / 10)):
            ...
            while not done:
                ...
                if replay_buffer.size() > minmal_size:
                    ...
                    agent.update(b_s, b_a, b_r, b_ns, b_d, max_seq, batch_size)
