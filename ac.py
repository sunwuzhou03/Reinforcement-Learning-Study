...
class AC:
    ...
    def update(self, transition_dict):
        ...
        q_now = self.critic(state).gather(1, action).view(-1, 1)
        q_next = self.critic(next_state).gather(1, next_action).view(-1, 1)
        # y_now = self.gamma * q_next + reward
        y_now = (self.gamma * q_next * (1 - done) + reward).view(-1, 1)
        # print(y_now)
        # print(q_next)
        ...
        critic_loss = torch.mean(F.mse_loss(y_now, q_now))
