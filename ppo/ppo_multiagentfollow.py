...
if __name__ == "__main__":
    ...
    next_obs=env.reset()
    next_obs = torch.tensor(next_obs,dtype=torch.float).to(args.device)
    ...
    for update in range(1,num_updates+1):
        ...
        for step in range(0,args.num_steps):
            ...
            if "episode" in info.keys():
                print(f"global_step={global_step}, episode_length={info['episode']['l']},episodic_return={info['episode']['r']}")
                writer.add_scalar("charts/episodic_return", np.array(info["episode"]["r"]).mean(), global_step)
                ...
                if info['episode']['e']%100==0:
                    ...
                    save_path = f"Follow/{timestamp}_{args.gym_id}.pth"
            ...
            buffer.rewards[step] = torch.tensor(reward[0]).to(args.device).view(-1)
            next_obs, next_done = torch.tensor(next_obs,dtype=torch.float).to(args.device), torch.tensor(done).to(args.device)
