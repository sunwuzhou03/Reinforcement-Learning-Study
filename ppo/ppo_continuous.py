...
def compute_advantage(buffer, args, next_obs, next_done):
    # 计算优势
    with (torch.no_grad()):
...
class Learner(nn.Module):
    def __init__(self, agent: PPOAgent, args):
        ...
        self.optimizer = torch.optim.Adam(agent.parameters(), args.learning_rate, eps=1e-5)
...
if __name__ == "__main__":
    ...
    agent = PPOAgent(env.observation_space, env.action_space).to(args.device)
    ...
    for update in range(1,num_updates+1):
        ...
        if args.anneal_lr:
            ...
            learner.optimizer.param_groups[0]["lr"] = lrnow
        ...
        for step in range(1,args.num_steps):
            ...
            # next_obs, reward, terminated, truncated, info = env.step(action.flatten().cpu().numpy())
        ...
        compute_advantage(buffer, args, next_obs, next_done)
        v_loss,pg_loss,entropy_loss,old_approx_kl,approx_kl,clipfracs_mean,explained_var=learner(buffer)

        # TRY NOT TO MODIFY: record rewards for plotting purposes
        writer.add_scalar("charts/learning_rate", learner.optimizer.param_groups[0]["lr"], global_step)
